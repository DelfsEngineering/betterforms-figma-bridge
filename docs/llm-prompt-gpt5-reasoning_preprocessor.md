...
- **Images** → Use appropriate placeholder service (NEVER use empty/broken src):
...
2. **Check for background layers first:**
   - Locked layers (especially named "BG" or "Background") → likely background
   - Full-size elements (same dimensions as parent) → likely background
   - If background detected → Use z-index pattern (see Issue Type 3)
3. **Analyze element grouping by position:**
   - Group elements with similar X coordinates (±50px tolerance) → vertical column
   - Group elements with similar Y coordinates (±50px tolerance) → horizontal row
   - Multiple distinct X positions → `flex flex-row` with child groups
   - Example: Text at X=100, Buttons at X=800 → horizontal layout with left/right groups
...
5. Determine layout pattern:
   ...
   - Background + content → `relative` parent + background layering
...
**🚨 CRITICAL - BetterForms wraps elements in divs which breaks natural stacking!**
  ...
  "fields": [
    {
      ...
      "styleClasses": "absolute inset-0 w-full h-full z-0 mb-0",  // ← Add z-0
      "type": "html"
    },
    {"html": "<h2>Heading</h2>", "styleClasses": "relative text-white ... mb-0"},  // ← Add relative
    {"html": "<p>Text</p>", "styleClasses": "relative text-white ... mb-0"},      // ← Add relative
    {"type": "group", "styleClasses": "relative flex gap-4 mb-0", "fields": [...]}  // ← Add relative
  ]
}
```
...
**Pattern:**
1. Parent container → Must have `relative`
2. Background element → Add `absolute inset-0 z-0`
3. ALL content elements → Add `relative` to their styleClasses
4. Content groups → Also need `relative`
...
**🚨 CRITICAL - BetterForms has default text styling that must be overridden!**
...
- Headings: `<h1>`, `<h2>`, `<h3>` → Must have `text-white`, `text-gray-900`, `text-[#color]`
- Paragraphs: `<p>` → Must have text color class
- Spans: `<span>` → Must have text color class
- Even if text is inside buttons or other components

**Use Tailwind classes, NOT inline styles:**
- ✅ `styleClasses: "text-white"`
- ✅ `styleClasses: "text-[#2d2f31]"`
...
**Solution:** Replace arbitrary values with Tailwind presets when there's an exact match (e.g., `bg-[#ffffff]` → `bg-white`, `text-[#000000]` → `text-black`). Only use arbitrary values `[...]` for custom colors/sizes that don't have presets.
...
**🚨 CRITICAL:** For background elements (SVG/gradients) with overlaid content, z-index IS required because BetterForms wraps each element in additional divs that break natural DOM stacking order. See Issue Type 3 for the required pattern.

---

### Font Awesome Icons

If `data-figma-font` contains "Font Awesome", convert ligature to FA class (e.g., `"edit"` → `<i class="fa-regular fa-edit"></i>`).
...
**✅ CORRECT:** Single object with `type`, `BFName`, `styleClasses`, `attributes` (wrapped in `formGroup`/`formElement`)
